...
export function CitySearch({ value, onChange, hasError }: Props) {
  const [query, setQuery] = useState('');
  const [open, setOpen] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  const selected = SA_CITIES.find((c) => c.name === value) ?? null;
  ...
  function select(city: typeof SA_CITIES[number]) {
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        {selected && !open ? (
          <span className="flex-1 text-zinc-100 truncate">
            {selected.name}
            <span className="text-zinc-500 ml-1.5 text-xs">{selected.province}</span>
          </span>
        ) : (
          <input
            type="text"
            ...
            placeholder={selected ? selected.name : 'Search for a city…'}
      ...
      {open && (
        <ul className="absolute z-50 mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-xl shadow-xl max-h-56 overflow-y-auto">
          {filtered.length === 0 ? (
            <li className="px-4 py-3 text-sm text-zinc-500">No cities found.</li>
          ) : (
            filtered.map((c) => (
              <li key={c.name}>
                <button
                ...
                >
                  ...
                  <span className="text-zinc-500 text-xs ml-auto shrink-0">{c.province}</span>
                </button>
              </li>
            ))
          )}
